"""eval_xboundary_vseries.py — X-Boundary adapter 를 **v70/v72 와 동일한 eval set**
...
왜 필요:
  eval_xboundary.py 는 raw dataset slice([64:164]) 기반 *unconditional* ASR (200 harmful
  pool 통째로 비거절율) → v-series 의 `jb_corr asr Δpp` (baseline 뚫린 sample 만 conditional)
  와 base population 다름 → 표에 같이 놓을 수 없음. 본 스크립트는 정확히 같은 5-group
  eval split 을 적용해서 paper main row 와 직접 비교.
...
사용:
  python experiment/xboundary/eval_xboundary_vseries.py \
      ...
      --out experiment/output/resp_xboundary_3b_vseries.json
...
def print_table(m: dict):
    ...
    print(f"  X-Boundary  jb_corr asr Δ = {jbc:+.2f}   "
...
def main():
    ap = argparse.ArgumentParser(
        description="X-Boundary LoRA eval (v-series eval set + group-conditional Δpp)")
    ...
    ap.add_argument("--out", default="experiment/output/resp_xboundary_3b_vseries.json")
    ap.add_argument("--metrics-out", default="output/metrics/xboundary_vseries_metrics.json",
                    help="v-series 규약 (output/metrics/<name>_metrics.json) — per_sample 제외 metric 만")
    ...
    result = {
        ...
        "config": {
            ...
            "method_meta": {
                ...
                "compared_against": "v68-v73 (same eval split + same WildGuard judge)",
    ...
    # 7) v-series 규약: output/metrics/<name>_metrics.json (per_sample 제외, by_group + overall + config)
    bg = metrics["by_group"]
    metrics_compact = {
        "baseline_off": {g: {"refuse_off": d["refuse_off"], "asr_off": d["asr_off"],
                             "n": d["n"]} for g, d in bg.items()},
        "adapter_on":   {g: {"refuse_on": d["refuse_on"], "asr_on": d["asr_on"],
                             "n": d["n"]} for g, d in bg.items()},
        "aggregate_diff": {g: {"refuse_delta_pp": d["refuse_delta_pp"],
                               "asr_delta_pp": d["asr_delta_pp"], "n": d["n"],
                               "skipped": d.get("skipped", 0)} for g, d in bg.items()},
        "overall": {
            "n_total": metrics["n_total"],
            "refuse_delta_pp": metrics["overall_refuse_delta_pp"],
            "asr_delta_pp": metrics["overall_asr_delta_pp"],
            "net_jb_asr_pp": metrics["net_jb_asr_pp"],
            "skipped_total": metrics.get("skipped_total", 0),
        },
        "vseries_row": {
            "method": "X-Boundary (3.2-3B, compute-matched)",
            "jb_corr_asr_delta_pp":    bg.get("jb_corr",    {}).get("asr_delta_pp"),
            "or_corr_refuse_delta_pp": bg.get("or_corr",    {}).get("refuse_delta_pp"),
            "jb_blocked_asr_delta_pp": bg.get("jb_blocked", {}).get("asr_delta_pp"),
            "net": metrics["net_jb_asr_pp"],
        },
        "config": result["config"],
    }
    save_atomic(args.metrics_out, metrics_compact)
    print(f"[done] n={len(per_sample)}  →  resp:    {args.out}  ({time.time() - t0:.0f}s)")
